...
class ImagePickerField extends HookWidget {
  ...
  _selectImage(BuildContext parentContext, ImagePicker picker,
      InfoFormState state) async {
  ...
  void _uploadImage(BuildContext parentContext, PickedFile pickedFile,
      InfoFormState state) async {
    if (pickedFile == null) {
      Scaffold.of(parentContext).showSnackBar(
          const SnackBar(content: Text("Nenhum arquivo foi selecionado")));
    } else {
      Scaffold.of(parentContext).showSnackBar(
          const SnackBar(content: Text("Salvando, aguarde um momento")));
      ...
      firebase_storage.UploadTask uploadTask =
          ref.putFile(io.File(pickedFile.path), metadata);
      uploadTask.whenComplete(() async {
        final url = await ref.getDownloadURL();
        parentContext
            .bloc<InfoFormBloc>()
            .add(InfoFormEvent.photoUrlChanged(url.toString()));
