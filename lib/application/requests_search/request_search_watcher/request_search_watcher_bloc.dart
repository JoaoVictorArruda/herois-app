...
@injectable
class RequestSearchWatcherBloc
    extends Bloc<RequestSearchWatcherEvent, RequestSearchWatcherState> {
  final IRequestRepository _requestRepository;

  RequestSearchWatcherBloc(this._requestRepository)
      : super(const RequestSearchWatcherState.initial());

  StreamSubscription<Either<RequestFailure, KtList<RequestSearch>>>
      _requestSearchStreamSubscription;
    ...
    yield* event.map(
      watchNearbyStarted: (e) async* {
        ...
        final RequestSearchFilter requestSearchFilter =
            RequestSearchFilterDto.fromFirestore(query.docs[0]).toDomain();
        ...
        _requestSearchStreamSubscription = _requestRepository
            ...
            .listen((request) => add(
                RequestSearchWatcherEvent.requestSearchReceived(
                    request, requestSearchFilter)));
      },
      requestSearchReceived: (e) async* {
        ...
        yield e.failureOrRequest.fold(
          ...
          (request) => RequestSearchWatcherState.loadSuccess(
              request, user.id.getOrCrash(), e.requestSearchFilter),
        );
      },
    );
  }
